...
mod test {
    ...
    #[test]
    fn a_selection_feeds_the_chat_once() {
        assert!(
            READER_CONTROLLER_JS.contains("removeAllRanges()"),
            "an unfocused frame hides its selection but keeps it, so reopening the chat \
             would draft and send the same passage again",
        );
    }
