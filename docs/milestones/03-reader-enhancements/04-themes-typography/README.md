...
showed the book's own CSS dictating colours and type with no reader control, and surfaced a
concrete bug (chapters render as a giant hover-red link). Both are addressed here. The
decision to **layer** reader styles over the book's CSS (rather than replace it), to render
content as **served XHTML**, and to **not fork `rbook`**, is
