...
**Status:** 🚧 in progress — captured 2026-09-11; [Phase 17](01-provider/phase-17-provider-boundary.md) opened the same day. No ADR yet. Sits behind
...
| 18 | API key storage + settings row | Key entered in the settings panel, stored in the OS keychain (not SQLite), read back on launch; provider/model chosen from a list | ⬜ |
| 19 | Chat panel | A route or drawer showing a message list and an input; sends through the provider; conversation held in memory | ⬜ |
| 20 | Selection → prefilled chat (desktop) | Read the selection out of the WebView via the existing eval bridge; a menu item opens the chat with the template filled in | ⬜ |
